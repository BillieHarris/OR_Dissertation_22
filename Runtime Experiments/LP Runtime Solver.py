...
# Linear Programming model, as used for other experiments. 
def LP_Solver(grid,m):
	'''
	Solves the sudoku puzzle via linear programming and records the time taken for the model to solve the sudoku 
    ...
    # Number of rows and columns in a submatrix.
    p = int(np.sqrt(m))
    ...
    # Defining the continuous decision variables.
    x = model.addVars(m, m, m, vtype=GRB.CONTINUOUS, name='x')
    
    # Setting lower and upper bounds for each decision variable.
    for i in range(m):
        for j in range(m):
            for k in range(m):
                x[i,j,k].lb = 0
