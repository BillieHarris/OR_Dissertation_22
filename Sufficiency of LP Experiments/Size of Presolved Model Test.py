# Conducting a combined experiment to determine if the size of the presolved model has an effect on whether a sudoku
# has integer solutions to the LP problem or not. 
...
def check_if_integer(sol,m,grid,count):
    ...
    Outputs:
    count: Binary variable - will return 0 if the solution contains fractions, and 
    ...
    # If at least one fractional value has been found, count is set to 0. 
    if is_integer == False:
        count = 0
    return(count)
...
def LP_solver(grid,m):
    ...
    if model.status != GRB.Status.INFEASIBLE:
        ...
        int_count = check_if_integer(solution,m,grid,int_count)
        
    return(int_count)

def presolve_size(grid,m):
    '''
    Determines if the presolved model for a sudoku puzzle is empty or not. 
    ...
    Outputs:
    empty_count: Binary variable - will return 1 if the presolved model is empty and 0 if not.   
    '''
    empty_count = 1
    ...
    # Update the above parameter of the model. 
    model.update()
    ...
    # Assign givens of the problem to their positions.
    # This is done by adjusting the lower bounds. 
    for i in range(m):
        for j in range(m):
            if grid[i][j] != '.':
                # Must subtract by 1 to account for the fact that indexing starts at 0. 
    ...
    # Querying the size of the presolved model
    pres = model.presolve()
    pres.Params.LogToConsole = 0
    presolve_rows = pres.numconstrs
    presolve_columns = pres.numvars
    
    # If the presolved model is non-empty, empty_count is set to 0. 
    if presolve_rows != 0 and presolve_columns != 0:
        empty_count = 0
...
# Initializes variables to count the number of sudoku puzzles within each of the cases. 
total_integer_and_empty_count = 0 
total_integer_and_not_empty_count = 0
total_no_int_and_empty_count = 0
total_no_int_and_not_empty_count = 0
...
while True:
    ...
    empty_count = presolve_size(sudoku,m)
    
    # Increments the correct variable by one depending on results from the functions. 
    if integer_count == 1 and empty_count == 1:
        total_integer_and_empty_count +=1
    elif integer_count == 1 and empty_count == 0:
        total_integer_and_not_empty_count +=1
    elif integer_count == 0 and empty_count == 1:
        total_no_int_and_empty_count += 1
    else:
        total_no_int_and_not_empty_count += 1
    
    
print("Number of sudoku solved to integer and presolved to empty:",total_integer_and_empty_count)
print("Number of sudoku solved to intger and not presolved to empty:",total_integer_and_not_empty_count)
print("Number of sudoku not solved to integer and presolved to empty:",total_no_int_and_empty_count)
print("Number of sudoku not solved to integer and not presolved to empty:",total_no_int_and_not_empty_count)
